    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="bg-[#1A56DB] px-6 pt-10 pb-20">
        ...
        <div className="flex flex-col items-center">
          ...
          <h2 className="text-white font-bold text-xl">Viktoria Angelita</h2>
          <p className="text-blue-200 text-sm mt-1">viktoria@email.com</p>
        </div>
      </div>
      ...
      <div className="px-6 mb-4">
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          {[
            { icon: "👤", label: "Nama Lengkap", value: "Viktoria Angelita" },
            { icon: "📧", label: "Email", value: "viktoria@email.com" },
            { icon: "📱", label: "No. Telepon", value: "+62 812 3456 7890" },
            { icon: "📍", label: "Kota", value: "Surabaya, Jawa Timur" },
          ].map((item, i, arr) => (
            <div
              key={item.label}
      ...
      {/* Riwayat Tiket */}
      <div className="px-6 mb-4">
        <p className="text-sm font-bold text-gray-700 mb-3">Riwayat Tiket</p>
        <div className="space-y-3">
          {riwayatTiket.map((tiket) => (
            <div key={tiket.id} className="bg-white rounded-2xl shadow-sm p-4">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-blue-50 rounded-xl flex items-center justify-center text-xl flex-shrink-0">
                  {tiket.emoji}
                </div>
                <div className="flex-1">
                  <h3 className="text-sm font-bold text-gray-800">{tiket.event}</h3>
                  <p className="text-xs text-gray-400 mt-0.5">{tiket.tanggal} · {tiket.kategori}</p>
                  <p className="text-xs text-gray-400">{tiket.kode}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold text-[#1A56DB]">{tiket.harga}</p>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-6 py-3">
        <div className="flex justify-around">
          {[
            ...
            { icon: "👤", label: "Profil", path: "/profil", active: true },
          ].map((item) => (
            <button
              key={item.label}
